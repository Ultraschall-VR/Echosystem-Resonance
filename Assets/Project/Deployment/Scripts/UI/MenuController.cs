    ...
    public class MenuController : MonoBehaviour
    {
        ...
        private void Initialize()
        {
            ...
            if (_gamestate == GameStateMachine.Gamestate.Orpheus || _gamestate == GameStateMachine.Gamestate.Loading ||
                _gamestate == GameStateMachine.Gamestate.LoadingOceanFloor)
            {
                _toggleMenu = true;
            }
        ...
        {
            Vector3 rot = new Vector3(_playerInput.Head.transform.eulerAngles.x,
                _playerInput.Head.transform.eulerAngles.y, 0);
        ...
        private void SetMenuPosition()
        {
            ...
            if (_gamestate != GameStateMachine.Gamestate.Orpheus)
            {
                ...
                {
                    pos = new Vector3(_playerInput.Head.transform.position.x,
                        _playerInput.Head.transform.position.y - 1, _playerInput.Head.transform.position.z);
                }
                else
                {
                    pos = new Vector3(_playerInput.Head.transform.position.x, 0,
                        _playerInput.Head.transform.position.z);
                }
            }
